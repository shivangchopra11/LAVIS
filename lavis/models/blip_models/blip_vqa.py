...
def compute_gradcam(model, sample, tokenized_text, block_num=6):
    ...
    with torch.no_grad():
        ...
        cams = cams[:, :, :, 1:].reshape(sample['image'].size(0), 12, -1, 30, 30) * mask
        grads = (
            grads[:, :, :, 1:].clamp(0).reshape(sample['image'].size(0), 12, -1, 30, 30)
            * mask
        )
        ...
        for ind in range(sample['image'].size(0)):
